$(document).ready(function(){
  ...
  $('body').append('<div id="callback-widget" style="'
        ...
        + 'height: 30px;'
        + '}'
        + '#callback-widget input, textarea {'
        + 'margin: 0;'
        + '}'
      ...
      + '<form border="0" name="contact" style="padding: 20px;" method="POST" action="" onsubmit="return(validate());">'
          ...
          + '<!--Phone number-->'
            ...
            + '<input type="text" name="phone" class="input" value="" maxlength="30">'
						+ '<label style="width: 48%; margin: 5px 10px 5px 0px;">Call Type *</label>'
          ...
          + '<!--Best to call-->'
            ...
            + '<select style="width: 48%; margin-bottom: 5px; margin-right: 10px;" id="callback_time_month" name="time_month">'
            ...
            + '</select>'
            + '<select style="width: 48%; margin-bottom: 5px;" id="callback_time_day" name="time_day">'
              + '<option value="1">1</option>'
              + '<option value="2">2</option>'
              + '<option value="3">3</option>'
              + '<option value="4">4</option>'
            ...
            + '</select>'
            + '<select style="width: 48%; margin-bottom: 5px; margin-right: 10px;" id="callback_time_hour" name="time_hour">'
            ...
            + '</select>'
            + '<select style="width: 48%; margin-bottom: 5px;" id="callback_time_gmt" name="time_gmt">'
            ...
            + ' border: 1px solid transparent; border-radius: 4px !important;'
            + ' background-color: #337ab7; border-color: #2e6da4;'
            + ' cursor: pointer; font-family: inherit; font-size: 13.33px;"'
            + ' name="submitc" value="Send request">'
            + '<label style="text-align: right;">Powered by <a href="https://call-back.io" style="color: #337ab7;"><b>call-back.io</b></a></label>'
      + '</form>'
    + '</div>'
    + '<script>'
      ...
      + '});'
			...
			+ 'console.log(dateObj.toString().match(/([A-Z]+[\\+-][0-9]+)/)[1]);'
			+	'$("#callback_time_month").val(month);'
			+	'$("#callback_time_day").val(day);'
			+	'$("#callback_time_gmt").val(dateObj.toString().match(/([A-Z]+[\\+-][0-9]+)/)[1]);'
			+ '});'
    + '</script>');
});
